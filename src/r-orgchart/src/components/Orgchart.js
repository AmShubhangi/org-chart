...
class OrgChart extends Component {
    ...
    render() {
        const groupedData = _.groupBy(this.props.data, 'ParentLocationId');
                        ...
                        <div className={`node-box ${this.props.animation ? 'node-animation' : ''} ${nodeClassName}`} style={nodeStyle} >
                            {editable &&
                                <div className={`org-node-btns ${btnsClassName}`} style={btnsStyle}>
                                    ...
                                    {(node.ParentLocationId !== null || (node.ParentLocationId === null && rootEditable)) &&
                            ...
                            <div onClick={(node.ParentLocationId !== null || (node.ParentLocationId === null && rootEditable)) && editable ? this.editNode : undefined} className="node-name-box" >
                                <p className="node-name" style={{ backgroundColor: this.state.bgColor }}>Name :{_.truncate(node.Name, { length: 35, omission: ' ...' })}</p>
                                <p className="node-name">Users:{_.truncate(node.Users, { length: 35, omission: ' ...' })}</p>
                                <p className="node-name">Identifiers:{_.truncate(node.ParentLocationId | 0, { length: 35, omission: ' ...' })}</p>
                            </div>
                        </div>
                }
            </ClickOutsideHandler>
